...
class WorkshopDatabase:
    ...
    def createWorkshopsTables(self) -> None:
        ...
        self.c.execute(
            """CREATE TABLE IF NOT EXISTS participantInformation (
                    id INTEGER PRIMARY KEY,
                    workshopID TEXT NOT NULL,
                    name TEXT NOT NULL,
                    email TEXT NOT NULL,
    ...
    def addWorkshop(self, wsDict: dict) -> None:
        ...
        self.c.execute(
            "INSERT INTO workshops (workshopID, workshopName, workshopStartDateAndTime, workshopSignedUp, workshopParticipantCapacity, workshopURL) VALUES (?,?,?,?,?,?)",
            (
                wsDict["workshopID"],
                wsDict["workshopName"],
                wsDict["workshopStartDateAndTime"],
                wsDict["workshopSignedUp"],
                wsDict["workshopParticipantCapacity"],
                wsDict["workshopURL"],
            ),
        )
        ...
        for participant in wsDict["workshopParticipantInfoList"]:
            self.c.execute(
                "INSERT INTO participantInformation (workshopID, name, email, school) VALUES (?,?,?,?)",
                (
                    wsDict["workshopID"],
                    participant["name"],
                    participant["email"],
    ...
    def getParticipantInfoForWorkshop(self, workshopID):
        ...
        return self.c.execute(
            "SELECT name, email, school FROM participantInformation WHERE workshopID = ?",
            (workshopID,),
        )
    ...
    def dropTables(self):
        ...
        self.c.execute(
            "DROP TABLE IF EXISTS participantInformation;",
        )
